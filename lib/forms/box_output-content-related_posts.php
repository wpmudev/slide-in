...
if ($show) {
	...
	foreach ($posts as $related) {
		$image = $related_has_thumbnails
			...
			: false
		;
		$out .= '<div class="slidein-col ' . ($related_has_thumbnails ? 'slidein-col-thumb' : '') . '">' .
			($related_has_thumbnails ? '<img class="slidein-thumb" src="' . $image . '" />' : '') .
			'<h2><a href="' . get_permalink($related->ID) . '">' . $related->post_title . '</a></h2>' .
			'<p>' . wdsi_get_related_post_excerpt($related) . '</p>' .
		'</div>';
	}
	echo '<div class="slidein-columns">' . $out . '</div>';
}